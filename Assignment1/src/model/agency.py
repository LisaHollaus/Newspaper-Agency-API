...
# editor:
    ...
    def remove_editor(self, editor: Editor):
        if len(editor.issues_list) != 0:  # if the editor had issues in his/her supervision
            for issue in editor.issues_list:
                for new_editor in self.editors:
                    newspaper_list = [x.newspaper_id for x in new_editor.issues_list]  # id_list of all newspapers a editor works for
    ...
    def get_editor_issues(self, editor):
...
# subscriber:
    ...
    def get_subscriber_stats(self, subscriber):
        ...
        return jsonify(f"Number of newspaper subscriptions: {newspaper_number} "
                       f"Cost: {monthly_cost} monthly or {monthly_cost * 12} annually "  # special issues not included (Assuming special issues are payed directly) 
